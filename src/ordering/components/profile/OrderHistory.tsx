...
export function OrderHistory() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold">{t('orders.history')}</h2>
        <p className="text-sm text-gray-600">{t('orders.viewAndTrack')}</p>
      </div>
      ...
      <div className="mb-6 space-y-4">
          ...
          <div className="w-full sm:w-auto">
            <MobileSelect
              options={[
                ...
                { value: 'highest', label: t('orders.sort.highest') },
                { value: 'lowest', label: t('orders.sort.lowest') }
              ]}
              value={sortOption}
        ...
        >
          ...
          {isFilterExpanded ? t('orders.filter.hide') : t('orders.filter.show')}
        </button>

        {/* Expandable filter section */}
        {isFilterExpanded && (
          <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
            <h3 className="text-sm font-medium text-gray-700 mb-3">{t('orders.filter.byStatus')}</h3>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <p className="mt-4 text-gray-600 font-medium">{t('orders.noOrders')}</p>
          <p className="text-sm text-gray-500 mt-2">{t('orders.noOrdersMessage')}</p>
        </div>
      ) : filteredOrders.length === 0 ? (
        <div className="text-center py-8 bg-white rounded-lg shadow-sm border border-gray-200">
          <Filter className="mx-auto h-10 w-10 text-gray-400" />
          <p className="mt-3 text-gray-600">{t('orders.noMatch')}</p>
          <button 
            onClick={() => {
              setStatusFilter('all');
          ...
          {filteredOrders.map(order => (
            <div key={order.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="p-4 border-b border-gray-100">
                <h4 className="text-xs font-medium text-gray-500 uppercase mb-2">{t('orders.items')}</h4>
                    ...
                    return (
                      <div key={index} className="flex justify-between items-start">
                        <div>
                          ...
                          {item.notes && (
                            <p className="text-xs text-gray-600">{t('orders.note')}: {item.notes}</p>
                          )}
                        </div>
              ...
              <div className="p-4">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                  <div className="flex items-center text-xs text-gray-600">
                    ...
                    {t('orders.pickup')}: {new Date(order.estimatedPickupTime || order.pickup_time || '').toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                  </div>
                  <p className="text-base font-medium">{t('orders.total')}: ${order.total.toFixed(2)}</p>
                </div>

                {(order.specialInstructions || order.special_instructions) && (
                  <div className="mt-3 pt-3 border-t border-gray-100">
                    <p className="text-xs text-gray-700 font-medium">{t('orders.specialInstructions')}:</p>
