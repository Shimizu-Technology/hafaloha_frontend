// src/ordering/wholesale/components/FundraiserCustomizationModal.tsx
import { useEffect, useState } from "react";
import { X, ChevronDown, ChevronUp } from "lucide-react";
import { useWholesaleCartStore } from '../store/wholesaleCartStore';
import type { FundraiserOptionGroup, FundraiserOption, SelectedOptionGroup, SelectedOption } from '../types/optionGroups';

interface FundraiserCustomizationModalProps {
  item: {
    id: number;
    name: string;
    price: number;
  ...
  optionGroups: FundraiserOptionGroup[];
  onClose: () => void;
}

export function FundraiserCustomizationModal({ 
  item, 
  fundraiserId, 
  participantId, 
  optionGroups, 
  onClose 
}: FundraiserCustomizationModalProps) {
  ...
  useEffect(() => {
    ...
    optionGroups.forEach(group => {
      const preselectedOptions = group.options
        .filter(opt => opt.is_preselected && opt.is_available) // Only include available options
        .map(opt => opt.id);
  ...
  function handleOptionToggle(group: FundraiserOptionGroup, opt: FundraiserOption) {
    setSelections((prev) => {
  ...
  // Note: Price breakdown calculations are now handled by paidOptionsByGroup directly
      ...
      if (chosenIds.length > freeCount) {
        ...
        const selectedOptions = chosenIds
          .map(id => {
            ...
            // Get the additional price for this option
            const price = option ? option.additional_price : 0;
  ...
  function isOptionFree(group: FundraiserOptionGroup, optId: number): boolean {
    const chosenIds = selections[group.id] || [];
    const freeCount = group.free_option_count || 0;
  ...
  // Get price breakdown for detailed view - used in the expanded price breakdown UI
  ...
  // On "Add to Cart": build a selectedOptions object => convert to SelectedOptionGroup format
  function handleAddToCart() {
    ...
    // Convert selections to SelectedOptionGroup format
    const selectedOptionGroups: SelectedOptionGroup[] = [];

    // For each group => collect the chosen options
    for (const group of optionGroups) {
      const chosenIds = selections[group.id] || [];
      if (chosenIds.length > 0) {
        const selectedOptions: SelectedOption[] = group.options
    ...
    addToCart({
      ...
      participantId,
      quantity,
      selectedOptions: selectedOptionGroups
    });
                  ...
                  {isExpanded && (
                    <div className="p-3 border-t border-gray-200">
                      ...
                      <div className="space-y-2">
                        {group.options
                          .filter(opt => opt.is_available) // Only show available options
                          .map((opt) => {
                          const selected = selections[groupId]?.includes(opt.id);
                          const extraPrice = opt.additional_price;
          ...
          <div className="mb-4">
            ...
            {!isPriceBreakdownExpanded ? (
              ...
              <div className="space-y-1 mb-3 animate-fadeIn">
                ...
                {/* Show breakdown from the paidOptionsByGroup, which is already calculated */}
...
export default FundraiserCustomizationModal;
